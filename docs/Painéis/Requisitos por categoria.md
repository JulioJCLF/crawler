---
tipo: painel
gerado_em: 2026-09-21
tags:
  - crawler/painel
---
# Requisitos por categoria

> [!info] Página gerada automaticamente em 2026-09-21. Mesma lista, organizada pela categoria de cada item.

## Backend (crawler)

| Requisito | Status |
|---|---|
| [[REQ-001 Monitorar as categorias do catálogo]] | 🚀 implementado |
| [[REQ-002 Percorrer todas as páginas de cada categoria]] | 🚀 implementado |
| [[REQ-003 Respeitar os limites do site de origem]] | 🚀 implementado |
| [[REQ-004 Descartar produtos fora do escopo]] | 🚀 implementado |
| [[REQ-008 Avisar as mudanças por webhook]] | ✅ aprovado |
| [[REQ-019 Detectar queda brusca de produtos numa categoria antes de publicar]] | 🚀 implementado |

## Vitrine

| Requisito | Status |
|---|---|
| [[REQ-011 Listar produtos com filtros e paginação]] | 🚀 implementado |
| [[REQ-012 Esconder itens sob consulta da vitrine]] | 🚀 implementado |
| [[REQ-013 Mostrar preço em reais com o preço anterior]] | 🚀 implementado |
| [[REQ-014 Oferecer contato direto e carrinho em cada card]] | 🚀 implementado |
| [[REQ-017 Mostrar filtros ativos e permitir limpá-los de uma vez]] | 🚀 implementado |

## Automação

| Requisito | Status |
|---|---|
| [[REQ-009 Rodar sozinho todo dia e guardar o resultado]] | 🚀 implementado |
| [[REQ-010 Publicar a vitrine automaticamente]] | 🚀 implementado |
| [[REQ-016 Garantir testes automatizados a cada mudança]] | 🚀 implementado |
| [[REQ-018 Avisar quando a execução diária falhar]] | 🚀 implementado |

## Dados e histórico

| Requisito | Status |
|---|---|
| [[REQ-005 Identificar cada produto e normalizar links]] | 🚀 implementado |
| [[REQ-006 Comparar com a execução anterior e classificar a mudança]] | 🚀 implementado |
| [[REQ-007 Manter o histórico de preços de cada produto]] | 🚀 implementado |
| [[REQ-015 Manter a lista de marcas indexadas editável]] | 🚀 implementado |

