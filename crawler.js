...
const CATEGORIES = config.crawlerCategories || [];
if (CATEGORIES.length === 0) {
  console.error("Nenhuma categoria configurada no crawlerCategories de config.json");
  process.exit(1);
}
...
function parseProducts(html, cat) {
  ...
  const scraper = config.scraper || {
    ...
    productIdRegex: "/produto/.*?-(\\d+)\\.html",
    priceRegex: "USD\\s*([\\d.,]+)",
    imageSelector: "img[src*=\"/produtos/\"]",
    baseUrl: "https://www.arsenalsports.com"
  };
  
  const idRx = new RegExp(scraper.productIdRegex);
  const priceRx = new RegExp(scraper.priceRegex);

  $(scraper.productLinkSelector).each((_, el) => {
    const href = $(el).attr("href") || "";
    const m = href.match(idRx);
    if (!m) return;
    const id = m[1];
    ...
    const priceMatch = card.text().match(priceRx);

    let img =
      card.find(scraper.imageSelector).first().attr("src") ||
      $(el).find("img").first().attr("src") ||
      null;
    if (img && !img.startsWith("http"))
      img = `${scraper.baseUrl}${img}`;

    byId.set(id, {
      id,
      name: title.replace(/\s+/g, " "),
      url: href.startsWith("http") ? href : `${scraper.baseUrl}${href}`,
